...
5. **재현 가능한 선택.** model tree, adapter, 명령, raw output, CSV를 SHA-256
   receipt로 연결하고, 더 많은 epoch/view가 항상 낫다는 가정을 실제 ablation으로
   반박했다.

최종 Public score는 **0.93542**다. Public은 test의 70%만 사용하므로 이 점수를
Private 또는 외부 데이터 성능의 보장으로 해석하지 않는다. 그 대신 외부 평가에도
동일한 checkpoint-2726과 동일 추론 코드를 한 번만 적용한다.
...
맞추기 위해 4-bit QLoRA를 사용했다. language attention/MLP projection에 rank32
adapter를 적용하고 base는 quantized 상태로 유지했다. batch 1, gradient accumulation
8, BF16 compute, image area 512×512 상당, seed 42를 사용했다. 최종 checkpoint는
global step 2,726이며 adapter tensor 512개가 모두 finite임을 확인했다.
...
명시적인 탐색 축으로 취급했다. 마지막 구간에서 2,726과 3,400을 보존하고 같은
generation 계열로 비교했다. checkpoint-3400 TTA12는 0.93542로 checkpoint-2726
Latin4를 넘지 못했다. 따라서 더 긴 학습을 “성능 향상”으로 포장하지 않고, 계산량이
더 작은 2,726을 채택했다.

### 외부 자원과 데이터

학습 데이터는 운영진 제공 데이터만 사용했다. 외부 상용 API, 외부 labeling API,
외부 데이터셋, 다른 모델의 pseudo-label, row-level model mixing은 사용하지 않았다.
Base model은 대회 cutoff 이전 공개된 Apache-2.0 Qwen3.6-27B 한 개다. RunPod RTX
A6000은 학습 장비로만, 로컬 RTX 3090은 최종 추론과 재현 검증에 사용했다.
...
번씩 나타나는 Latin square다. 따라서 input-position exposure는 네 view만으로
완전 균형이다. 24개 모든 permutation은 coverage를 늘리지만 이미 position bias가
0인 이후에는 noisy view를 더하며 계산량만 6배가 될 수 있다.
...
| Q36 checkpoint | step1074 Latin4 confidence | 0.93019 | 학습 진행으로 개선 |
...
- 최종 inference: 약 3.75 GPU-hours

Provider invoice와 가정하지 않은 전기·감가상각 비용은 정확한 총액으로 환산하지
않았다. 계산 절약의 핵심은 Latin4, single checkpoint, unmerged adapter,
...
`metrics.json`으로 저장된다. raw audit를 보존해 parser와 집계를 독립 재계산할 수
있다. 채점된 핵심 source SHA-256은 inference `7ab46f47…d414`, TTA
